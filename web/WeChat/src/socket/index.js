...
function socket (commit, actions) {
  ...
  this.socketUrl = process.env.VUE_APP_SCOKET
  this.$soctket_init()
}
...
socket.prototype.$soctket_init = function (callback) {
  ...
  const roleId = store.getters.userChosoeRoleID
  ...
  const errorDispatch = (eventment) => {
    ...
    return function () {
      ...
      event === 1 ? console.log('web socket has failed  from closeState (断开)') : console.log('web socket has failed  from errorState ')
  ...
  _this.ws = new WebSocket(_this.socketUrl + '?sid=' + sid + '&roleId=' + roleId)
  ...
  _this.ws.onopen = function () {
    ...
    console.log('web socket has connected（已连接） ')
  }
...
socket.prototype.$soctket_emit = function (value, callback) {
  ...
  if (!_this.ws) {
    throw new Error('$socket dispatch is fail please use $socket_open method（断连）')
  }
...
socket.prototype.$soctket_subscribe = function () {
  ...
  _this.ws.onmessage = function (res) {
    console.log('接收成功')
    store.dispatch('socket/saveSuccesse', JSON.parse(res.data))
    _this.$soctket_heartSoctket()
  }
}
/**
...
socket.prototype.$soctket_open = function () {
  if (!this.closeWs) {
    throw new Error('socket is connected（重连）')
  }
  this.timer = null
